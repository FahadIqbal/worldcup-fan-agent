...
/**
 * Load full user context from Neon at session start.
...
export async function loadUserContext(userId: string): Promise<UserContext> {
  try {
    ...
    return {
      profile: (profileRes.rows?.[0] as UserProfile) ?? null,
      activeTrips: (tripsRes.rows as Trip[]) ?? [],
      activeAlerts: (alertsRes.rows as PriceAlert[]) ?? [],
      recentRuns: (runsRes.rows as AgentRun[]) ?? [],
    };
  } catch {
...
/**
 * Build the system context message injected at conversation start.
 */
...
/**
 * Upsert user profile (called after first message if profile incomplete).
 */
...
/**
 * Persist agent run log to Neon after goal completion.
 */
...
/**
 * Clean up sessions older than 2 hours from in-memory map.
 */
export function cleanupStaleSessions(): void {
  const TWO_HOURS = 2 * 60 * 60 * 1000;
  const now = Date.now();
  for (const [id, state] of sessions.entries()) {
    if (now - new Date(state.startedAt).getTime() > TWO_HOURS) {
      sessions.delete(id);
    }
  }
}
