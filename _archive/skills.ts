...
You are building a complete trip plan. Work through these steps in order:
...
📍 Pro tip: Arrive 90 min early for fan zones — queues are long on match days.`,
  },
...
Option 3 — {mode}: {duration}, ~${cost}, {notes}
...
/**
 * Classify a user message to the best matching skill(s).
 * Returns an ordered array — first element is primary skill.
 */
export function classifyIntent(message: string): Skill[] {
  ...
  // If nothing matched, default to SK-01 (Trip Planner)
  if (matched.length === 0) {
    return [SKILLS["SK-01"]];
  }
...
/**
 * Detect compound goals that require multiple skills.
 * e.g. "Plan my trip AND alert me when flights drop below $900"
 */
export function detectCompoundGoal(message: string): Skill[] {
  ...
  // Return top 2 skills max for compound goals
  return skills.slice(0, 2);
}

/**
 * Build the full system prompt for a given skill.
 */
