// app/api/jobs/price-check/route.ts
// Cloud Scheduler job — runs every 6h to check flight price alerts
...
async function verifySchedulerToken(req: NextRequest): Promise<boolean> {
  ...
  // In production: verify with Google's token info endpoint
  // For dev: accept a shared secret
  ...
  for (const alert of alerts) {
    try {
      ...
      await neonUpsert("price_alerts", {
        ...
        triggered,
        active: !triggered, // deactivate once triggered
      });
...
async function sendPushNotification(userId: string, body: string) {
  // Get FCM token from Neon
  const res = await neonQuery(
    `SELECT fcm_token FROM user_profiles WHERE id = '${userId}'`
  );
...
async function sendEmailNotification(userId: string, subject: string, body: string) {
  const res = await neonQuery(
    `SELECT email FROM user_profiles WHERE id = '${userId}'`
  );
