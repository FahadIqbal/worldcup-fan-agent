...
MODEL_NAME="$(grep ^GEMINI_MODEL .env.local | cut -d= -f2-)"
...
store_secret GEMINI_MODEL         "$MODEL_NAME"
...
gcloud run deploy $SERVICE_NAME \
  ...
  --cpu=1 \
  --memory=1Gi \
  --set-env-vars="NODE_ENV=production,GCP_PROJECT_ID=$PROJECT_ID,GCP_REGION=$REGION,GEMINI_MODEL=$MODEL_NAME,GEMINI_MAX_OUTPUT_TOKENS=4096,GEMINI_TEMPERATURE=0.3,PHOENIX_COLLECTOR_ENDPOINT=https://app.phoenix.arize.com,PHOENIX_PROJECT=worldcup-fan-agent" \
